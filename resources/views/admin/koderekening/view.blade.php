                                                ...
                                                <td>
                                                    <div class="dropdown">
														...
														<div class="dropdown-menu">
                                                            @if ($d->status == '1')
                                                            <a type="button" class="dropdown-item status" data-id="{{Crypt::encrypt($d->id_rekening)}}"> <i class="fa fa-toggle-on color-muted"></i> Aktif</a>
                                                            @else
                                                            <a type="button" class="dropdown-item status" data-id="{{Crypt::encrypt($d->id_rekening)}}"> <i class="fa fa-toggle-off color-muted"></i> Nonaktif</a>
                                                            @endif
															<a type="button" class="dropdown-item edit" href="#" data-id="{{Crypt::encrypt($d->id_rekening)}}"> <i class="fa fa-pencil color-muted"></i> Edit</a>
															<a type="button" class="dropdown-item hapus" href="#" data-id="{{Crypt::encrypt($d->id_rekening)}}" ><i class="fa fa-trash color-muted"></i> Hapus</a>
														</div>
													</div>
                                                </td>
                                            @endforeach
...
@push('myscript')
    ...
    $(document).on('click', '.edit', function(){
        ...
        $.ajax({
                        type: 'POST',
                        url: '/admin/kegiatan/koderekening/edit',
                        cache: false,
                        data: {
                            _token: "{{ csrf_token() }}",
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        window.location = "/admin/kegiatan/koderekening/hapus"+id_rekening
      }
    });
    });
    </script>
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        window.location = "/admin/kegiatan/koderekening/status"+id_rekening
        }
      });
    });
    </script>
